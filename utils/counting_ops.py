...
def resnet50_FLOPs(sparse_init: str = "random", density: float = 0.2,) -> int:
...
def print_resnet50_stats():
    dense_FLOPs = resnet50_FLOPs(density=1.0)
    ...
    for density in [0.1, 0.2]:
        Random_FLOPs = resnet50_FLOPs("random", density)
        ER_FLOPs = resnet50_FLOPs("erdos-renyi", density)
        ERK_FLOPs = resnet50_FLOPs("erdos-renyi-kernel", density)
...
if __name__ == "__main__":
    print_wrn_22_2_stats()
    print_resnet50_stats()
