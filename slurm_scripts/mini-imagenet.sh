...
if [ ${1} == "RigL" ]; then
  ...
  masking=RigL +specific=mini-imagenet_resnet50_masking \
...
if [ ${1} == "SNFS" ]; then
  python main.py hydra/launcher=basic dataset=Mini-Imagenet optimizer=SGD \
  masking=SNFS +specific=mini-imagenet_resnet50_masking \
...
if [ ${1} == "SET" ]; then
  python main.py hydra/launcher=basic dataset=Mini-Imagenet optimizer=SGD \
  masking=SET +specific=mini-imagenet_resnet50_masking \
...
if [ ${1} == "Static" ]; then
  python main.py hydra/launcher=basic dataset=Mini-Imagenet optimizer=SGD \
  +specific=mini-imagenet_resnet50_static \
...
if [ ${1} == "Small-Dense" ]; then
  python main.py hydra/launcher=basic dataset=Mini-Imagenet optimizer=SGD \
  masking=Small_Dense +specific=mini-imagenet_resnet50_dense \
  seed=$SLURM_ARRAY_TASK_ID masking.density=0.05,0.1,0.2,0.5 wandb.use=True -m
fi

if [ ${1} == "Pruning" ]; then
  python main.py hydra/launcher=basic dataset=Mini-Imagenet optimizer=SGD \
  +specific=mini-imagenet_resnet50_pruning exp_name='Pruning' \
  seed=$SLURM_ARRAY_TASK_ID  masking.final_density=0.05,0.1,0.2,0.5 wandb.use=True -m
fi

if [ ${1} == "Lottery" ]; then
  python main.py hydra/launcher=basic dataset=Mini-Imagenet optimizer=SGD masking=Lottery \
  +specific=mini-imagenet_resnet50_lottery exp_name='Lottery' \
  seed=$SLURM_ARRAY_TASK_ID  masking.density=0.05,0.1,0.2,0.5 wandb.use=True -m
fi
#wait