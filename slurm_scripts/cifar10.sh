...
if [ ${1} == "RigL-SG" ]; then
  if [ ${2} == "ERK" ]; then
    python main.py dataset=CIFAR10 optimizer=SGD masking=RigL \
    +specific=cifar10_wrn_22_2_masking seed=$SLURM_ARRAY_TASK_ID exp_name="RigL-SG_ERK" \
  ...
  if [ ${2} == "Random" ]; then
    python main.py dataset=CIFAR10 optimizer=SGD masking=RigL \
    +specific=cifar10_wrn_22_2_masking seed=$SLURM_ARRAY_TASK_ID exp_name="RigL-SG_Random" \
    masking.redistribution_mode=grad masking.print_FLOPs=true \
    masking.sparse_init=random masking.density=0.05,0.1,0.2,0.5 wandb.use=True -m
  fi
fi
...
if [ ${1} == "RigL-SM" ]; then
  if [ ${2} == "ERK" ]; then
    python main.py dataset=CIFAR10 optimizer=SGD masking=RigL \
    +specific=cifar10_wrn_22_2_masking seed=$SLURM_ARRAY_TASK_ID exp_name="RigL-SM_ERK" \
  ...
  if [ ${2} == "Random" ]; then
    python main.py dataset=CIFAR10 optimizer=SGD masking=RigL \
    +specific=cifar10_wrn_22_2_masking seed=$SLURM_ARRAY_TASK_ID exp_name="RigL-SM_Random" \
    masking.redistribution_mode=momentum masking.print_FLOPs=true \
    masking.sparse_init=random masking.density=0.05,0.1,0.2,0.5 wandb.use=True -m
  fi
fi
...
if [ ${1} == "RigL-reinit" ]; then
  if [ ${2} == "ERK" ]; then
    python main.py dataset=CIFAR10 optimizer=SGD masking=RigL_reinit \
    +specific=cifar10_wrn_22_2_masking_reinit seed=$SLURM_ARRAY_TASK_ID exp_name="RigL-reinit_ERK" \
  ...
  if [ ${2} == "Random" ]; then
    python main.py dataset=CIFAR10 optimizer=SGD masking=RigL_reinit \
    +specific=cifar10_wrn_22_2_masking_reinit seed=$SLURM_ARRAY_TASK_ID exp_name="RigL-reinit_Random" \
    masking.init_exp_name=RigL-SM_Random \
    masking.density=0.05,0.1,0.2,0.5 wandb.use=True -m
  fi
fi

if [ ${1} == "RigL-reinit-dist" ]; then
  if [ ${2} == "ERK" ]; then
    python main.py dataset=CIFAR10 optimizer=SGD masking=RigL_reinit \
    +specific=cifar10_wrn_22_2_masking_reinit seed=$SLURM_ARRAY_TASK_ID exp_name="RigL-reinit-dist_ERK" \
    masking.init_exp_name=RigL-SM_ERK masking.sparse_init=lottery-ticket-dist \
    masking.density=0.05,0.1,0.2,0.5 wandb.use=True -m
  fi

  if [ ${2} == "Random" ]; then
    python main.py dataset=CIFAR10 optimizer=SGD masking=RigL_reinit \
    +specific=cifar10_wrn_22_2_masking_reinit seed=$SLURM_ARRAY_TASK_ID exp_name="RigL-reinit-dist_Random" \
    masking.init_exp_name=RigL-SM_Random masking.sparse_init=lottery-ticket-dist \
    masking.density=0.05,0.1,0.2,0.5 wandb.use=True -m
  fi
fi
